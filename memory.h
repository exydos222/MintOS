// BUGS : POINTER INVALIDATION FOR MEMORY ALLOCATED BEFORE A REALLOCATION CALL
...
void deallocate(const void* address, const MEMORY_BLOCK_DATA_TYPE size) {
    for (struct memory_block* block = first_block; block; block = block->next)
        if ((unsigned int)address >= (unsigned int)block->data_pointer && (unsigned int)address <= (unsigned int)block->data_pointer + MEMORY_BLOCK_SIZE) {
            for (MEMORY_BLOCK_DATA_TYPE i = (unsigned int)address - (unsigned int)block->data_pointer + size; i < block->used; i++)
    ...
    for (struct memory_block* block = first_block; block; block = block->next)
        if ((unsigned int)address >= (unsigned int)block->data_pointer && (unsigned int)address <= (unsigned int)block->data_pointer + MEMORY_BLOCK_SIZE)
            if (old_size >= new_size) {
                for (MEMORY_BLOCK_DATA_TYPE i = (unsigned int)address - (unsigned int)block->data_pointer + old_size; i < block->used; i++)
                    ((char*)block->data_pointer)[i - old_size + new_size] = ((char*)block->data_pointer)[i];
                ...
                if (block->used - old_size + new_size > MEMORY_BLOCK_SIZE) {
                    ...
                    const MEMORY_BLOCK_DATA_TYPE old_end = (unsigned int)address - (unsigned int)block->data_pointer + old_size;
                    for (MEMORY_BLOCK_DATA_TYPE i = old_end; i < block->used; i++) {
